<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modifier le dossier de :') }} {{ $employee->nom_complet }}
        </h2>
    </x-slot>
    
    <div class="flex justify-end">
        <form action="{{ route('employees.destroy', $employee) }}" method="POST" onsubmit="return confirm('Etes vous sur ?');">
            @csrf
            @method('DELETE')
            <button type="submit" class=" font-medium text-white bg-red-500 hover:underline  hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">
                Supprimer cet agent définitivement de la base de donnée
            </button>
        </form>
    </div>
        ...
        <form method="POST" action="{{ route('employees.update', $employee) }}">
            ...
            <div class="grid grid-cols-2 gap-8 mb-4">
                <div class="">
                    ...
                    <input type="text" id="matricule" name="matricule" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="matricule de l'élément" value="{{ $employee->matricule }}">
                    @error('matricule')
                ...
                <div class="">
                    ...
                    <input type="text" id="nom_complet" name="nom_complet" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="écrire le nom" value="{{ $employee->nom_complet }}">
                    @error('nom_complet')
                        <span class="text-sm text-red-600 ">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-2">
                    ...
                    <input type="date" id="engage_le" name="engage_le" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ \Carbon\Carbon::parse($employee->engage_le)->format('Y-m-d')}}">
                </div>
                <div class="mb-2 bg-purple-600 rounded-md opacity-80">
                    <label for="depart_le" class="block mb-2 pl-2 text-sm font-medium text-gray-900">Départ le </label>
                    <input type="date" id="depart_le" name="depart_le" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" >
                </div>
                <div class="mb-2">
                    <label for="taille" class="block mb-2 pl-2 text-sm font-medium text-gray-900">Taille</label>
                    <input type="text" id="taille" name="taille" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="(en mètre)" value="{{ $employee->taille }}">
                </div>
                <div class=""></div>
                <div class="">      
                ...
                <div class="mb-2 col-start-1 col-end-3">                    
                    ...
                    <textarea id="description" name="description" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="note spécial sur l'agent">{{ $employee->adresse }}</textarea>
                </div>             
            </div>
